...
- **Dedicated Data Panels**: Each metric has a dedicated full-width panel containing visual tracking and a detailed History Table with raw tracking data:
  ...
  2. **Workouts, Personal Records (PR Grid) & Dettaglio Sessioni**: Bacheca a card dei **Record Personali (All-Time)** per ciascun esercizio svolto. Per gli esercizi con carico vince il peso massimo sollevato in qualsiasi serie valida con ripetizioni completate $>0$ (inclusi i test massimali al 100% eseguiti durante la rampa di riscaldamento `riscaldamento_effettivo`, e a parità di carico le ripetizioni maggiori), mentre per gli esercizi a corpo libero vince il numero massimo di ripetizioni. Supporta sia i dati strutturati `scheda_dati` (sia come array sia come oggetto `{ tipo, esercizi: [...] }`) che il parsing retroattivo intelligente delle note libere, con normalizzazione automatica dei nomi degli esercizi composti (es. *"Panca piana con bilanciere"* normalizzato in *"Panca Piana"*). Le date sono formattate con anno compatto (`DD/MM/YY`, es. `21/09/26`) sia sulle card dei PR che nella tabella Storico Sessioni. Cliccando su qualsiasi riga dello Storico Sessioni si apre la modale interattiva `#nst-modal-dettaglio-allenamento` che visualizza il breakdown dettagliato dell'allenamento (metadati, RPE, note, target previsti, rampa di riscaldamento con carichi e serie di lavoro effettive).
